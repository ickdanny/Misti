...
MidiSequence *midiSequenceAllocAndParseMidiFile(
    const char *fileName
){
    MidiSequence *midiSequencePtr = pgAlloc(
        1,
        sizeof(*midiSequencePtr)
    );
    if(midiSequencePtr){
        *midiSequencePtr = parseMidiFile(fileName);
    }
    return midiSequencePtr;
}
void midiSequenceFreeAlloc(
    MidiSequence *midiSequencePtr
){
    if(midiSequencePtr){
        midiSequenceFree(midiSequencePtr);
        pgFree(midiSequencePtr);
    }
}